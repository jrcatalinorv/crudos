...
<h3 class="card-title">  Kilos Auditados  </h3>
</div>
...
$stmt  = Conexion::conectar()->prepare("SELECT * FROM dk1_crudos2 WHERE fecha between ".$start_date." AND ".$end_date." ORDER BY rollo ASC ");
$stmt -> execute();	
...
//Realiza el Select en este espacio 
$stmt2  = Conexion::conectar()->prepare("SELECT COUNT(rollo) as CANT FROM crudos.dk1_crudos_defectos_rollo WHERE rollo = '".$rollo."'");
$stmt2 -> execute();	
$results2 = $stmt2 -> fetch();
$cant_defectos = $results2['CANT'];

if( intval( $cant_defectos ) > 1 ){
$stmt3  = Conexion::conectar()->prepare("SELECT * FROM crudos.dk1_crudos_defectos_rollo WHERE rollo = '".$rollo."'");
$stmt3 -> execute();
...
echo "<tr>";
	...
	echo "<td>".$turno."</td>";
	echo "<td>".$operador."</td>";
	echo "<td>".$results3['defecto']." </td>";
	echo "<td>".$results3['puntos']." </td>";
